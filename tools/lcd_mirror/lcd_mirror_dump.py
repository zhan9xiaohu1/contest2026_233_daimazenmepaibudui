...
# -*- coding: utf-8 -*-
"""无头接收器：收 lcd_mirror 的帧，按脏矩形拼到画布上，存成 PNG 供肉眼/工具检查。
用法: py -3.10 lcd_mirror_dump.py [秒数] [端口] [前缀]
"""
import socket, struct, sys, time
from PIL import Image

secs = int(sys.argv[1]) if len(sys.argv) > 1 else 15
port = int(sys.argv[2]) if len(sys.argv) > 2 else 5601
prefix = sys.argv[3] if len(sys.argv) > 3 else "mir"
...
while time.time() < end:
    ...
    try:
        fr = recv_frame(conn)
        if fr is None:
            conn.close(); conn = None; continue
        if fr == "bad":
            print("BAD MAGIC", flush=True)
            conn.close(); conn = None; continue
        x, y, w, hgt, seq, flags, ver, pix = fr
        bytes_total += header_size(ver) + len(pix)
        frames += 1
        ver_flags[(ver, flags)] = ver_flags.get((ver, flags), 0) + 1
        rects[(w, hgt)] = rects.get((w, hgt), 0) + 1
        if len(hdr_seen) < 8:
            hdr_seen.append((ver, flags, x, y, w, hgt, seq))
        if (x, y, w, hgt) == (0, 0, W, H):
            full_frames += 1
        else:
            partial_frames += 1
        t_dec = time.perf_counter()
        im = decode_frame(ver, flags, w, hgt, pix)
        dec_us_total += (time.perf_counter() - t_dec) * 1e6
        canvas.paste(im, (x, y)) if x + w <= W and y + hgt <= H else None
        now = time.time()
        if frames == 3 or (shots < 2 and frames and frames % 25 == 0):
            shots += 1
            canvas.resize((W * 2, H * 2), Image.NEAREST).save(prefix + "_%d.png" % shots)
    except socket.timeout:
        continue
    except OSError as e:
...
canvas.resize((W * 2, H * 2), Image.NEAREST).save(prefix + "_final.png")
print("frames=%d bytes=%d full=%d partial=%d" % (frames, bytes_total, full_frames, partial_frames))
print("ver/flags 计数:", sorted(ver_flags.items()))
print("平均解码 %.2f ms/帧" % ((dec_us_total / frames / 1e3) if frames else 0.0))
print("rect sizes (top):", sorted(rects.items(), key=lambda kv: -kv[1])[:10])
print("first headers (ver,flags,x,y,w,h,seq):", hdr_seen)
print("saved", prefix + "_final.png")
