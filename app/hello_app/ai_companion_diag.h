 ...
 *
 * 三条纪律（和 ai_companion_yield.h 是同一套）：
 ...
 *   empty  连续空读次数
 *   hold   让路已受理（正在让 / 已经让出去）   want  我要常听的意图
 *   busy   正拿着设备开关（让路半程）          req   让路请求方向（0 无 1 让 2 收）
 *         （2026-09-15 补的两条口径：busy 会在让路线程被判死时被主循环**强制清零**
 *           —— 半程标志挂在一条不回来的线程上只会一直挡着监听守护，配合 wdead 看；
 *           req 变成 2 也可能是**我们自己**改写的：让路太久没人收回时看门狗会
 *           替调用方登记一次收回，配合 wd 看 —— 这时候 hold 会跟着从 1 变 0、
 *           want 变 1。）
 *   net    MQTT 上报通道可用（g_net_started）
 *   wd     让路看门狗强制收回过（本次让路里调用方漏了 / 丢了 RECLAIM）
 ...
 *
 * 实现拆两半（照 ai_companion_yield.c / .h 的做法）：
 ...
 * "已经听上了"）、录音那几个标志之间的先后、以及"让路线程正在动设备"读到的那
