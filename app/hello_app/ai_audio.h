 ...
 *         ≥ AUDIO_RECORD_IDLE_DEAD_MS(180 秒)"同样算命中** —— 它兜住本函数
 *         看不见的那种形状：线程卡在数据回调里、压根没回到 read（这时本函数
 *         恒为 -1，光靠 ② 发现不了）。这个数必须**大于最长合法阻塞**
 *         （本构建 TLS socket 读超时 120 秒 + agent 层看门狗 60 秒），
 *         取小了就会在"正常但慢的一次对话"里误重开；
 *       · 合法的长空窗必须让开：扬声器在响、追问相位非空闲、以及**"对话
 *         （ASR + 大模型）正在跑且还没超过上界"** —— 这三个窗口里"没有数据"
 *         都是正常的（ASR/大模型同步跑在录音线程的回调里，TTS 出声时录音
 *         本来就被停掉了），不让开就是每次正常对话之后都白重开一次麦。
 *         ⚠️ 对话那条**必须带时限**（上界 AUDIO_DIALOGUE_MAX_BLOCK_MS，与
 *         上面那个 180 秒同源）：不带时限的话，那边一旦永不返回，这条判据
 *         就被它自己永久关掉，录音端又变回"永久聋"—— 判据的排除项和判据本身
 *         一样要有上界。
 *     阈值别再立第二个数：这里列的每个值都直接复用监听守护既有的常量，
 *     否则"最坏多久能自愈"就要看几个互相不知道对方存在的数。
