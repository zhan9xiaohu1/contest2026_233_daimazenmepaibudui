 ...
 *   持有者线程还在（别的线程/app 真在录）→ -EBUSY，板级不动那个会话。
 *   持有者线程已经不存在（死掉的 app/线程留下的残留，单一大镜像里这份全局没人
 *   替它清）→ 判为残留：丢掉记录后当成全新一次会话重开。要重录请先
 *   audio_in_stop()。
 *
 *   **遇 EBUSY 怎么办（本次改动，原来是"绝不先停再开"）**：
 *     会先停再开（只做一次）的条件 —— 占用者被**判定为残留**，即那条通路的持有
 *     者线程已经不存在（TCB 都查不到）。判定分两层，都在证据所在的那一层做：
 *       · 板级这一层：自己的记录（fd + 持有线程）里持有线程已消失 → 丢掉记录按
 *         全新会话重开，连 STOP 都不用发；
 *       · 驱动那一层：方向冲突时驱动会查自己记下的持有者线程，已经不存在就把那条
 *         残留通路干净 stop 掉再把方向切过来，对上层**透明**（本次 START 返回 0）。
