 ...
 *   - read() 会一直阻塞到读满；**被 STOP 打断 / 会话换代**返回 0（EOF）、
 *     **设备没在跑**返回 -ENODEV、等不到数据（下层那一次等待超时）返回
 *     -ETIMEDOUT。三者语义不同、收尾动作也不同：0 = "别人把设备拿走了"
 *     （只 close、**别**发 STOP），-ENODEV = "设备压根没在跑"（要 STOP + close
 *     把设备收干净），-ETIMEDOUT = "这一帧没数据"（跳过这一帧接着读）；
 ...
 *     > 0 = 真读到了这么多字节；
 *     = 0 = EOF：被 AUDIOIOC_STOP 打断 / 会话换代 ——
 *           这一代会话已经结束了，要跳出循环、把麦克风还回去；
 *     < 0 = 错误或超时。其中 errno = ENODEV 是**"设备没在跑"**（2026-09-20 晚
 *           从 EOF 里单独拆出来的码：处置和 EOF **正好相反** —— EOF 只 close，
 *           ENODEV 必须 STOP + close 把设备收干净，否则框架那份共享 status
 *           会一直被钉着，后面每个会话的 CONFIGURE/START 都被静默吞掉）；
 *           errno = ETIMEDOUT 是**下层那一次等待超时**
 ...
 *       audio_in_read() 返回 0/EOF 之后的收尾就是这种情况。0 的两个来源
 *       （见 audio_in_read）分别是"被 AUDIOIOC_STOP 打断 / 会话换代"：前一个是
 *       别人（或框架）把这一代停了，后一个是别人的会话已经接手了设备。
 *       两种情况下再发 STOP 都是纯破坏：AUDIOIOC_STOP 是
 *       **设备级**的，会同时停掉播放和录音，于是"被接手方"的收尾动作会把刚
 *       接手设备的那个会话打死 —— 单向让路变成双向拆台。
 *       ⚠️ 2026-09-20 晚：**"设备没在跑"不再属于这一支** —— 它现在由
 *          audio_in_read() 用 -ENODEV 单独报出来，处置正好相反：那一支**必须**
 *          走 stop（把设备收干净），否则框架那份共享 status 一直被钉着，
 *          后面每个会话的 CONFIGURE/START 都被静默吞掉。见 ai_audio.c 里
 *          -ENODEV 那一支的说明。
